...
output_dir="${REPORT_DIR}/duckwing-v67-joint-fusion"
mkdir -p "${output_dir}/drive" "${output_dir}/policies" "${output_dir}/evaluations" "${output_dir}/logs"

for path in "${incumbent}" "${specialist}" "${brake}" "${baseline}"; do
...
for steering in 0.00 0.10 0.25 0.50 0.75 1.00; do
    for propulsion in 0.50 0.75 0.90 1.00 1.10; do
        for head in 0.00 0.50 1.00; do
            tag="s${steering/./}-p${propulsion/./}-h${head/./}"
            ...
            if [[ ! -f "${evaluation}" ]]; then
                "${UV_BIN}" run python "${LAB_ROOT}/tools/evaluate_swizzle.py" \
                    ...
                    --line-hold --line-yaw-kp 0.70 --line-lateral-kp 0.14 \
...
printf 'V67 joint-fusion sweep complete: %s strict race-promotable policies. Results: %s\n' \
